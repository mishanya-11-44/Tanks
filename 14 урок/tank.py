...
class Tank:
    __count=0
    def __init__(self, canvas, x, y, model='T-34', ammo=100, speed=1,bot=True):
        self.__bot=bot
        self.__target=None
    ...
    def __take_speed(self):
        self.__usual_speed=self.__usual_speed*2
        self.__water_speed=self.__water_speed*2
    ...
    def __on_map_collision(self,details):
        if world.WATER in details and len(details)==1:
            ...
            if world.take(pos['row'],pos['col'])!= world.AIR:
                self.__take_ammo()
        elif world.NEW_BONUS in details:
            pos=details[world.NEW_BONUS]
            if world.take(pos['row'],pos['col'])!=world.AIR:
                self.__take_speed()
    ...
    def died(self):
        self.__canvas.itemconfig(self.__id,image=skin.get('tank_destroyed'))
    def update(self):
        if self.__hp==0:
            self.died()
    ...
    def __del__(self):
        try:
            ...
            pass
    def __str__(self):
        return (f"Модель: {self.__model}, Здоровье: {self.__hp}, "
                f"Опыт: {self.__xp}, Боеприпасы: {self.__ammo}, "
                f"Координаты: (x={self.__x}, y={self.__y})")